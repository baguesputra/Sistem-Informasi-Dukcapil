...
class surat_tugas {
    ...
    { 
        ...
        $data=mysqli_query($conn,"SELECT surat_tugas.id, surat_tugas.id_pegawai,surat_tugas.tanggal_awal, surat_tugas.tanggal_akhir, surat_tugas.tujuan, surat_tugas.rangka, surat_tugas.status, surat_tugas.transportasi, surat_tugas.pembiayaan,
       pegawai.nama, pegawai.nik, jabatan.jabatan, golongan.golongan, golongan.tingkat, golongan.ket FROM surat_tugas join pegawai on pegawai.id = surat_tugas.id_pegawai join jabatan on jabatan.id = pegawai.jabatan join golongan on golongan.id = pegawai.golongan"); 
    ...
    { 
        $koneksi = new koneksi();
        $conn =  $koneksi->get_connection();
        $data=mysqli_query($conn,"SELECT * FROM detail_surat_tugas join pegawai on detail_surat_tugas.id_pegawai = pegawai.id"); 
        while($d = mysqli_fetch_array($data)){
            $view[] = $d;
        } 
        return $view; 
    ...
    { 
        $koneksi = new koneksi();
        $conn =  $koneksi->get_connection();
        $sql = "INSERT INTO surat_tugas (perintah, id_pegawai,tujuan, tanggal_awal, tanggal_akhir, rangka, transportasi, pembiayaan , status) 
    ...
    public function tambah_detail($id_pegawai, $id_surat_tugas) 
    { 
        $koneksi = new koneksi();
        $conn =  $koneksi->get_connection();
        $sql = "INSERT INTO detail_surat_tugas (id, id_surat_tugas, id_pegawai) 
        VALUES ('$id', '$id_surat_tugas', '$id_pegawai')";
        $conn->query($sql);
    } 
    ...
    public function update($id,$perintah,$id_pegawai,$tujuan, $tanggal_awal, $tanggal_akhir, $rangka, $transportasi, $pembiayaan , $status)
    {
        $koneksi = new koneksi();
        $conn =  $koneksi->get_connection();
        $sql = "UPDATE surat_tugas SET perintah='$perintah', id_pegawai='$id_pegawai',tujuan='$tujuan', tanggal_awal='$tanggal_awal', 
    ...
    { 
        $koneksi = new koneksi();
        $conn =  $koneksi->get_connection();
        $sql = "UPDATE surat_tugas SET status='$status' WHERE id='$id'";
        $conn->query($sql);
    } 
    ...
    { 
        $koneksi = new koneksi();
        $conn =  $koneksi->get_connection();
        $sql = "DELETE FROM detail_surat_tugas WHERE id='$id'";
        $conn->query($sql);
    } 
}
?>
